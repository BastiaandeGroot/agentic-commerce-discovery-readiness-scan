  ...
  return (
    <div className="space-y-8">
      ...
      <section className="max-w-2xl">
        <h1 className="text-2xl font-semibold tracking-tight">{s.pages.home.title}</h1>
        <p className="mt-3 text-base leading-relaxed text-muted">{s.pages.home.intro}</p>
        <div className="mt-6 flex flex-wrap items-center gap-3">
          ...
          <Link
            href="/scan"
            className="rounded-lg border border-line px-3.5 py-2 text-sm font-medium transition hover:bg-surface-2"
          >
            {s.shell.nav.scan}
          </Link>
        </div>
        <p className="mt-3 text-sm text-muted">{s.pages.home.secondary}</p>
      </section>
